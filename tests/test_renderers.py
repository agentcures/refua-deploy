...
def test_render_public_bundle(tmp_path: Path) -> None:
    ...
    configmap_payload = yaml.safe_load(
        (output_dir / "kubernetes" / "configmap.yaml").read_text()
    )
    assert configmap_payload["kind"] == "ConfigMap"
    assert configmap_payload["data"]["REFUA_CAMPAIGN_OPENCLAW_BASE_URL"].startswith(
        "https://"
    )
    ...
    command = cronjob_payload["spec"]["jobTemplate"]["spec"]["template"]["spec"][
        "containers"
    ][0]["command"]
    assert command[0:2] == ["ClawCures", "run-autonomous"]
    claim_name = cronjob_payload["spec"]["jobTemplate"]["spec"]["template"]["spec"][
        "volumes"
    ...
    secret_docs = list(
        yaml.safe_load_all(
            (output_dir / "kubernetes" / "secrets.template.yaml").read_text()
        )
    )
...
def test_render_public_bundle_with_mcp_service_mode(tmp_path: Path) -> None:
    ...
    service_payload = yaml.safe_load(
        (output_dir / "kubernetes" / "mcp-service.yaml").read_text()
    )
    assert service_payload["spec"]["type"] == "LoadBalancer"

    ingress_payload = yaml.safe_load(
        (output_dir / "kubernetes" / "mcp-ingress.yaml").read_text()
    )
    assert ingress_payload["spec"]["ingressClassName"] == "nginx"

    mcp_deployment = yaml.safe_load(
        (output_dir / "kubernetes" / "mcp-deployment.yaml").read_text()
    )
...
def test_render_private_bundle(tmp_path: Path) -> None:
    ...
    compose_payload = yaml.safe_load(
        (output_dir / "private" / "docker-compose.yaml").read_text()
    )
...
def test_render_gpu_required_for_kubernetes_and_compose(tmp_path: Path) -> None:
    ...
    mcp_deployment = yaml.safe_load(
        (k8s_output / "kubernetes" / "mcp-deployment.yaml").read_text()
    )
    mcp_resources = mcp_deployment["spec"]["template"]["spec"]["containers"][0][
        "resources"
    ]
    ...
    cronjob = yaml.safe_load(
        (k8s_output / "kubernetes" / "campaign-cronjob.yaml").read_text()
    )
    campaign_resources = cronjob["spec"]["jobTemplate"]["spec"]["template"]["spec"][
        "containers"
    ][0]["resources"]
